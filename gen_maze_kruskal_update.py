"""Generating and Solving 2D Matrices

CS5800 Final Project

By Arushi Agarwal & Lindsey Alexanian"""
...
# generate text files for visualizing the maze
with open('maze_edges3.txt', 'w') as f:
    for i in matrix:
        for j in i:
            f.write('\t'.join((str(j[0]) + str(j[1]))) + '\n')

with open('maze_coordinate_pairs3.txt', 'w') as f:
    for i in matrix:
        #print(i[0], i[1])
        for j in i[0]:
            f.write('\t'.join(str(j)) + '\t')
...
class Point_Maze():
    ...
    def make_maze_dict(self):
        ...
        # make dictionary of maze points
    ...
    def final_maze_gen(self):
        ...
        # initialize final list of lists output
        final_maze = []
...
class Adjacency:
    ...
    def make_adj_list(self):
        ...
        # use list to make dictionary keys
        adj_list = dict.fromkeys(adj_list_keys)

        # initialize list in dict values
...
class Solve_Me:
    ...
    def bfs(self, source, sink):
        ...
        """This method runs a breadth-first search on the maze input"""
    ...
    def solve_maze_dfs(self):
        ...
        """This method calls the dfs() and compile() functions to
        solve the maze and get the shortest route path"""
...
if __name__ == "__main__":
    ...
    with open('maze_edge_letters.txt', 'w') as f:
        for i in my_maze:
            f.write('\t'.join(i) + '\n')
    ...
    # subtract to find time of algorithm
    total1 = t1 - t0
